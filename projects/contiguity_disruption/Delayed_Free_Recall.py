...
for model_index, model_class in enumerate(models):
    ...
    for ll_index, list_length in enumerate(list_lengths):
        ...
        sim_df_path = results_path + 'VARIABLE_LIST_LENGTHS_{}_{}_ll{}_sim_df.csv'.format(
            section_tag, model_names[model_index], list_length)
        if False: #os.path.isfile(sim_df_path):
            sim_df = pd.read_csv(sim_df_path)
            print('LIST LENGTHS sim_df for {} with list length {} and tag {} already exists'.format(
                model_names[model_index], list_length, section_tag))
    ...
    for analysis_index, analysis_function in enumerate(analyses):
        ...
        if analysis_name == 'crp':
            axis.set_ylim((0, .5))
        # plt.savefig(results_path+'VARIABLE_LIST_LENGTHS_{}_{}_ll{}_{}.pdf'.format(section_tag, model_names[model_index], list_length, analysis_name), bbox_inches="tight")
        plt.show()
# %%
