...
for model_index, model_class in enumerate(models):
    ...
    for delay_index, delay in enumerate(delays):
        ...
        sim_df_path = results_path + '{}_{}_{}_delay{}_sim_df.csv'.format(
            title, section_tag, model_names[model_index], str(int(np.round(delay))*10))
        if False: #os.path.isfile(sim_df_path):
            sim_df = pd.read_csv(sim_df_path)
            print('{} sim_df for {} with delay {} and tag {} already exists'.format(
                title, model_names[model_index], delay, section_tag))
        ...
        # otherwise, generate it
            ...
            for subject in pd.unique(individual_fits.subject):
                ...
                # manipulate delay drift rate parameter to match specified delay
                fitted_parameters['delay_drift_rate'] = delay
                ...
                subject_specific_sim_dfs[-1]['delay'] = delay
    ...
    for analysis_index, analysis_function in enumerate(analyses):
        ...
        axis = analysis_function(
            sim_dfs, 'subject > -1', contrast_name="delay", labels=delays)
