...
for model_index, model_class in enumerate(models):
    ...
    for practice_index in range(2):
        ...
        sim_df_path = results_path + '{}_{}_{}_practice{}_sim_df.csv'.format(
            title, section_tag, model_names[model_index], practice_index)
        if False: #os.path.isfile(sim_df_path):
            sim_df = pd.read_csv(sim_df_path)
            print('{} sim_df for {} with practice_index {} and tag {} already exists'.format(
                title, model_names[model_index], practice_index, section_tag))
        ...
        # otherwise, generate it
            ...
            for subject in pd.unique(individual_fits.subject):
                ...
                elif practice_index == 1:
                    presentationsA = np.expand_dims(np.arange(list_length, dtype=int), axis=0)
                    presentationsB = np.expand_dims(np.arange(list_length, dtype=int), axis=0).T
                    np.random.shuffle(presentationsB)
                ...
                subject_specific_sim_dfs[-1]['practice_index'] = practice_index
