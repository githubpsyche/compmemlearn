...
__all__ = ['alternative_contiguity', 'fast_crp', 'plot_lag_crp', 'fast_mixed_crp', 'randomize_dataset',
           'indices_of_repeated_items', 'alternative_contiguity_test', 'alternative_contiguity_control',
           'sim_alternative_contiguity_test', 'fast_pfr', 'plot_pfr', 'fast_mixed_pfr', 'flex_mixed_pfr', 'fast_spc',
           'plot_spc', 'recall_by_second_study_position', 'recall_by_all_study_positions', 'alternative_contiguity',
           'fast_csp', 'plot_csp', 'fast_crp', 'plot_lag_crp', 'fast_mixed_crp', 'randomize_dataset',
           'indices_of_repeated_items', 'alternative_contiguity_test', 'alternative_contiguity_control',
           'sim_alternative_contiguity_test', 'fast_pfr', 'plot_pfr', 'fast_mixed_pfr', 'flex_mixed_pfr', 'fast_rpl',
           'rpl', 'fast_spc', 'plot_spc', 'recall_by_second_study_position', 'recall_by_all_study_positions']
...
def alternative_contiguity(trials, presentations, lag_threshold = 3, max_repeats = 2):
    ...
    total_actual_lags = np.zeros((max_repeats, lag_range * 2 + 1)) # extended dimension to split by presentation positions
    ...
    for trial_index in range(len(trials)):
        ...
        for recall_index in range(terminus[trial_index]):
            ...
            # track possible and actual lags; focus only on transitions from items with > 1 study positions
            if recall_index > 0 and np.count_nonzero(
                possible_positions[previous_item]) > 1 and (
                possible_positions[previous_item][1] - possible_positions[previous_item][0] > 3):
                ...
                current_item_index = np.nonzero(possible_items==current_item)[0][0]
                ...
                for x in range(len(recall_by_study_position)):
                    for y in range(len(recall_by_study_position)):
                        ...
                        possible_lags[index][possible_positions[possible_items, y] == 0] = 0

                        actual_lag = possible_lags[index, current_item_index] + lag_range
    ...
    total_possible_lags[total_actual_lags[0]==0] += 1
    ...
    for trial_index in range(len(trials)):
        ...
        for recall_index in range(terminus[trial_index]):
            ...
            if recall_index > 0:
                ...
                if np.any(actual_lags[:, trial_index, recall_index-1] == 4):
                    print(trial_index, recall_index)
