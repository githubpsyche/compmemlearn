# AUTOGENERATED! DO NOT EDIT! File to edit: projects/03_Classic_CMR.ipynb (unless otherwise specified).

__all__ = ['icmr_spec', 'Instance_CMR', 'cmr_spec', 'Classic_CMR']
...
class Instance_CMR:
    ...
    def __init__(self, item_count, presentation_count, parameters):
        ...
        self.context = np.eye(1, 2*(item_count+2), item_count+2).flatten()
        self.preretrieval_context = self.context
        self.recall = np.zeros(item_count, int32) # at most `item_count` entries
        ...
        self.context_weighting[item_count:] = self.primacy_scale * np.exp(
            -self.primacy_decay * np.arange(presentation_count)) + 1
        ...
        self.memory = np.zeros(
            (item_count + presentation_count, item_count * 2 + 4))
    ...
    def update_context(self, drift_rate, experience):
        ...
        context_state = self.context[self.item_count+2:]
        rho = np.sqrt(1 + np.square(drift_rate) * (np.square(context_state * context_input) - 1)) - (
                drift_rate * (context_state * context_input))
        context_state = (rho * context_state) + (drift_rate * context_input)
        #context_state = context_state/np.sqrt(np.sum(np.square(context_state))) # norm to length 1
        self.context[self.item_count+2:] = context_state
    ...
    def outcome_probabilities(self):
        ...
        if self.probabilities[0] < (1.0 - ((self.item_count-self.recall_total) * 10e-7)):
            ...
            activation = self.echo(self.context)[1:self.item_count+1]
    ...
    def free_recall(self, steps=None):
        ...
        while self.recall_total < steps:
            ...
            if np.any(outcome_probabilities[1:]):
                choice = np.sum(np.cumsum(
                    outcome_probabilities) < np.random.rand(), dtype=int32)
            else:
                choice = 0
