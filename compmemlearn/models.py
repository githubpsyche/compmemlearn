...
__all__ = ['familiarity_weighting', 'cmr_spec', 'Classic_CMR', 'familiarity_weighting', 'cmr_spec', 'Classic_CMR_Alpha',
           'Dual_ICMR', 'dual_icmr_spec', 'Base_CMR', 'base_cmr_spec', 'Single_ICMR', 'single_icmr_spec', 'icmr_spec',
           'Dual_Instance_CMR', 'expcmr_spec', 'Exp_ICMR', 'Lrn_ICMR', 'icmr_de_spec', 'ICMR_DE', 'icmr_spec',
           'Instance_CMR', 'icmr_spec', 'Instance_CMR_Alpha', 'ncmr_spec', 'noise', 'Noisy_CMR', 'trcmr_spec',
...
class Dual_ICMR:
    def __init__(self, item_count, presentation_count, parameters):
        ...
        mcf_pre1 = np.hstack(
            (np.zeros((item_count, 1)), mcf_pre1, np.zeros((item_count, 1)))
        )
        mcf_pre2 = np.hstack(
            (np.zeros((item_count, 1)), mcf_pre2, np.zeros((item_count, 1)))
        )

        self.mfc = np.zeros((item_count + presentation_count, item_count * 2 + 4))
        self.mfc[
            :item_count,
        ] = np.hstack((mfc_pre, mcf_pre1))
        self.mcf = np.zeros((item_count + presentation_count, item_count * 2 + 4))
        self.mcf[
            :item_count,
        ] = np.hstack((mfc_pre, mcf_pre2))
        ...
        self.items = np.hstack(
            (
                np.eye(item_count, item_count + 2, 1),
                np.zeros((item_count, item_count + 2)),
            )
        )
    ...
    def activations_mfc(self, probe, probe_norm=1.0):
        ...
        ):  # if probe is an item feature cue as during contextual retrieval
            if np.any(
                probe[self.item_count + 2 :]
